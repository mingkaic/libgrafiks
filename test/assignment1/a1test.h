...
public:
	...
	virtual void run (size_t testid, Drawable* drawable, PANEL_DRAW panel_drawer) const
	{
		...
		DRAW draw = [drawable](int x, int y, double z, double zp, unsigned int color, glib::normal& n)
		{
		...
		[this, opacity, drawable](int x, int y, double z, double zp, unsigned int color, glib::normal& n)
		{
			unsigned int oldColor = drawable->getPixel(x, y);
